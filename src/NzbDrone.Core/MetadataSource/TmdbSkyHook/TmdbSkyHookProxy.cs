    ...
    public class TmdbSkyHookProxy : IProvideSeriesInfo, ISearchForNewSeries
    {
        ...
        public Tuple<Series, List<Episode>> GetSeriesInfo(int tvdbSeriesId)
        {
            try
            {
                ...
                var tvShow = TmdbClient.GetTvShowAsync(tmdbSeriesId, tvShowExtraMethods).Result;
                var fanartTv = GetFanartTv(tvdbSeriesId);

                var tvSeasonEpisodes = new List<TvSeasonEpisode>();
                foreach (var season in tvShow.Seasons)
                {
                    var tvSeason = TmdbClient.GetTvSeasonAsync(tmdbSeriesId, season.SeasonNumber).Result;
        ...
        private Series MapSearchResult(SearchTv searchTv)
        {
            ...
            if (series == null)
            {
                ...
                var tvShow = TmdbClient.GetTvShowAsync(searchTv.Id, tvShowExtraMethods).Result;
        ...
        private Series MapSeries(TvShow show, FanartTv fanartTv)
        {
            ...
            series.Seasons = show.Seasons.Select(season => MapSeason(season, fanartTv, show.OriginalLanguage)).ToList();
            ...
            if (fanartTv != null)
            {
                var seriesbanner = GetBestOptionForFanartTvArt(fanartTv.tvbanner, CountryCode, show.OriginalLanguage);
                if (seriesbanner != null)
                {
                    series.Images.Add(new MediaCover.MediaCover(MediaCoverTypes.Banner, seriesbanner.url));
                }

                var seriesclearart = GetBestOptionForFanartTvArt(fanartTv.hdclearart, CountryCode, show.OriginalLanguage);
                if (seriesclearart != null)
                {
                    series.Images.Add(new MediaCover.MediaCover(MediaCoverTypes.ClearArt, seriesclearart.url));
                }

                var seriesclearlogo = GetBestOptionForFanartTvArt(fanartTv.hdtvlogo, CountryCode, show.OriginalLanguage);
                if (seriesclearlogo != null)
                {
                    series.Images.Add(new MediaCover.MediaCover(MediaCoverTypes.ClearLogo, seriesclearlogo.url));
                }

                var serieslandscape = GetBestOptionForFanartTvArt(fanartTv.tvthumb, CountryCode, show.OriginalLanguage);
        ...
        private Season MapSeason(SearchTvSeason seasonResource, FanartTv fanartTv, string showOriginalLanguage = null)
        {
            ...
            if (fanartTv != null)
            {
                if (fanartTv.seasonbanner != null)
                {
                    ...
                    {
                        var seasonbanner = GetBestOptionForFanartTvArt(seasonbanners.Cast<FanartTvArt>().ToList(), CountryCode, showOriginalLanguage);
                ...
                if (fanartTv.seasonthumb != null)
                {
                    ...
                    {
                        var seasonthumb = GetBestOptionForFanartTvArt(seasonthumbs.Cast<FanartTvArt>().ToList(), CountryCode, showOriginalLanguage);
                        if (seasonthumb != null)
                        {
        ...
        private static FanartTvArt GetBestOptionForFanartTvArt(List<FanartTvArt> fanarts, string preferedLanguageTwoLetterCode = null, string originalLanguageTwoLetterCode = null)
        {
            FanartTvArt bestFanArt = null;

            if (fanarts != null && fanarts.Any())
            {
                if (preferedLanguageTwoLetterCode.IsNotNullOrWhiteSpace())
                {
                    bestFanArt = fanarts.FirstOrDefault(art => art.lang == preferedLanguageTwoLetterCode);
                }

                if (bestFanArt == null && originalLanguageTwoLetterCode.IsNotNullOrWhiteSpace())
                {
                    bestFanArt = fanarts.FirstOrDefault(art => art.lang == originalLanguageTwoLetterCode);
                }
                ...
                if (bestFanArt == null)
                {
                    bestFanArt = fanarts.FirstOrDefault();
                }
            }
